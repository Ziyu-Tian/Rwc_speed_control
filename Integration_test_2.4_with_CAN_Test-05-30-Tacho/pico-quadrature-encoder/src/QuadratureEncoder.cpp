...
int32_t QuadratureEncoder::get_last_count()
{
    return _last_count;
}
...
{
    // _last_count = quadrature_encoder_get_count(pio_instance, sm);
    // sleep_ms(100);
    // _count = quadrature_encoder_get_count(pio_instance, sm);
    float velocity = ((_count - _last_count) / (ppr * gear_ratio *_dt)*1000);

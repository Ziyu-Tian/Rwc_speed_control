...
float get_median_rpm(float current_rpm) {
    median_buf[median_idx] = current_rpm;
...
void core0_entry()
{
    ...
    QuadratureEncoder encoder(pio1, proximity_sensor_pin, ppr);
    // QuadratureEncoder encoder_2(pio0, A2_pin, ppr);
    // QuadratureEncoder encoder_3(pio1, A3_pin, ppr);

    gpio_init(proximity_sensor_pin);
    gpio_set_dir(proximity_sensor_pin, GPIO_IN);
    gpio_pull_up(proximity_sensor_pin);
    ...
    {   
        ...
        int rpm_smoothed = get_median_rpm(rpm);
        ...
        // printf("Last_count: %d ",last_count);
        // printf("RPM_smooth: %d ",rpm_smoothed);
        // printf("RPM: %d ",rpm);
        // printf("Count: %d\n",counter);
        
        // // Tacho Code
        // bool current_state = gpio_get(proximity_sensor_pin);
